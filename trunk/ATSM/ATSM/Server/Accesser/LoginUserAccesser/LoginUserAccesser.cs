...
namespace JVL
{
    public class LoginUserAccesser
    {
        ...
        public LoginUser CheckLogin(String id,String password, Boolean evict)
        {
            IQuery query = session.CreateQuery("select c from LoginUser as c where c.LoginUserId= :x and c.Password = :y ");
            query.SetString("x", id);
            query.SetAnsiString("y", password);
            LoginUser s = (LoginUser)query.UniqueResult();
            if (evict)
            {
                session.Evict(s);
