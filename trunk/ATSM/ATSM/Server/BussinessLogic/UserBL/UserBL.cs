...
namespace JVL
{
    public class UserBL
    {
        ...
        public UserWSI SaveObject(UserWSI wsi)
        {
            ...
            {
                //Insert code at here
                ATSMUser user = new ATSMUser();
                ATSMUserAccesser userAccesser = new ATSMUserAccesser(Session);
                user.Id = wsi.ATSMUser.Id;
                user.Password = wsi.ATSMUser.Password;
                userAccesser.InsertATSMUser(user);
                tx.Commit();
            }
            catch (Exception ex)
            {
